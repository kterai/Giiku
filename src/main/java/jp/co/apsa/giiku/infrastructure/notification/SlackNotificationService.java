...
public class SlackNotificationService implements NotificationPort {
     ...
     * Slack Webhook URL
     */
    @Value("${giiku.slack.webhook-url}")
    private String webhookUrl;

    /**
     * Slack チャンネル名（デフォルト）
     */
    @Value("${giiku.slack.default-channel:#general}")
    private String defaultChannel;

    /**
     * 緊急通知用チャンネル名
     */
    @Value("${giiku.slack.urgent-channel:#urgent}")
    private String urgentChannel;

    /**
     * 通知送信者名
     */
    @Value("${giiku.slack.bot.username:Giiku System}")
    private String botUsername;

    /**
     * 通知送信者アイコン
     */
    @Value("${giiku.slack.bot.icon::robot_face:}")
    private String botIcon;

    /**
     * リトライ最大回数
     */
    @Value("${giiku.slack.retry.max-attempts:3}")
    private int maxRetryAttempts;

    /**
     * リトライ間隔（ミリ秒）
     */
    @Value("${giiku.slack.retry.delay:1000}")
    private long retryDelay;

    /**
     * 通知有効フラグ
     */
    @Value("${giiku.slack.enabled:true}")
