...
### 最近の更新
- 2025-09-05: 学生回答テーブルをV000に統合しDDL集約ルールを追加
- 2025-09-04: 理解度テスト・演習回答機能とリアルタイム監視ダッシュボードを実装
- 2025-09-03: ダッシュボードにクイズ回答送信フォームを追加
- 2025-09-02: WebSocketを用いたリアルタイム回答配信機能を追加
...
### 概要
- 講義ごとに理解度テストと演習問題を表示し、受講者の回答をデータベースへ保存。
- 講師と受講者はダッシュボードで回答状況と成績を即時確認可能。

### 操作手順
1. ダッシュボードの「理解度テスト」メニューを開く。
2. 出題されるクイズまたは演習問題に回答し、「送信」をクリック。
3. 結果は自動的に保存され、成績画面に反映される。

## 📡 リアルタイム監視機能

### 概要
- WebSocketによるリアルタイム監視で、受講者の回答状況を即座に追跡。
- 管理者・講師は監視ダッシュボードで学習進捗をリアルタイムに把握可能。

### 操作手順
1. 管理者ダッシュボードの「監視」タブにアクセス。
2. 受講者の回答が送信されると、一覧が自動更新される。
3. 必要に応じて詳細画面から個別の学習ログを確認する。
...
### データベースマイグレーション
- DDLの変更はすべて `V000__Create_Users_And_Companies.sql` に集約し、以降のマイグレーションはデータ操作のみとする。
```bash
# 新しいマイグレーションファイルの作成
